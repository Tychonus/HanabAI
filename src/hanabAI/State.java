...
/**
 * Represent the game state in Hanabi. The state is designed as an immutable
 * object. Agents are able to get the current game state, and all previous moves
...
public class State implements Cloneable {
  ...
  public State(String[] players, Stack<Card> deck) throws IllegalArgumentException {
    ...
    for (Colour c : Colour.values())
      fireworks.put(c, new Stack<Card>());
  ...
  /**
   * A method to create the next state from the given state and a move. This
   * method will only work if no hand has been hidden (since it allows agents to
   * infer the result of actions).
   * 
   * @param deck   the deck of cards
   * @param action the action made
   * @throws IllegalActionException if the move is not legal in the current state,
   *                                or one hand has been hidden.
   **/
  ...
  /**
   * A method to create a local state from a global state. That is, if there is no
   * current observer, the specified observer will have their hand hidden from
   * them.
   * 
   * @param observer the player observing the game state
   * @throws IllegalActionException if the observer is out of bounds, or if the
   *                                state is not global.
   **/
  ...
  /**
   * Test the legality of a Action. If the observer of a state is specified, this
   * mathod can only be applied to actions performed by the observer.
   * 
   ...
   * 
   * @return an array containing the naems of the players in the game, by ther
   *         index in the game.
   **/
   ...
   * @return an array of cards in player's hand, or an empty array, if the cards
   *         are hidden.
  ...
  public Card[] getHand(int player) throws ArrayIndexOutOfBoundsException {
    ...
    if (player == observer)
      return new Card[hands[player].length];
  ...
  /**
   * Gives the previous state of the game, allowing agents to determine the recent
   * actions in the game.
   * 
   ...
   * @throws ArrayIndexOUtOfBoundsException if the specified player has not yet
   *                                        performed an action
   **/
  public Action getPreviousAction(int player) {
    ...
    while (s != null && s.previousAction != null && s.previousAction.getPlayer() != player)
      s = s.previousState;
    if (s == null || s.previousAction == null)
      throw new ArrayIndexOutOfBoundsException("Player has not played yet");
    else
      return s.previousAction;
  }

  /**
   * Gets the card played in the previous move, or null if it is the first move,
   * or the previous move was a hint.
   * 
   * @return the card played in the previous action, or null if there is no
   *         previous action, or the action was a hint.
   */
  ...
  public Stack<Card> getDiscards() {
    return (Stack<Card>) discards.clone();
  }

  /**
   * Get the stack of cards representing the specified firework
   * 
   * @return a clone of the stack of cards representing the firework of the given
   *         colour. The highest card is at the top of the stack.
   **/
  ...
  /**
   * Returns the order of the final action, if it is known. When the final card is
   * drawn from the deck, every player has one more action remaining. The order of
   * the final action, is the order of the action that drew the final card, plus
   * the number of players in the game.
   * 
   * @return the order of the final action, or -1 if the deck is not empty
   **/
  public int getFinalActionIndex() {
    return finalAction;
  }
   ...
   * 
   * @return true if all fireworks have been made, the deck has run out, or a fues
   *         has exploded.
   **/
  ...
  /**
   * Returns a string describing the state of the game, including: the state of
   * each players hand; the top card of each fire work; and the last action.
   */
