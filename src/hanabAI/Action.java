...
/**
 * An abstract class to represent actions in the game Hanabi. The class is
...
public class Action {
   ...
   * @param type       the type of the action, should be ActionType.PLAY or
   *                   actionType.DISCARD
   * @param pos        the position of the card to be discarded in the players
   *                   hand
  ...
  public Action(int player, String playerName, ActionType type, int pos) throws IllegalActionException {
    ...
    if (type != ActionType.PLAY && type != ActionType.DISCARD)
      throw new IllegalActionException("Wrong parameters for action type");
    card = pos;
  }
   ...
   * @param cards        an array of booleans, such that the ith value is true if
   *                     and only if the ith card in the hintee's hand matches the
   *                     hint
   * @param hint         the colour hinted at.
   * @throws IllegalActionException if the wrong ActionType is given
   */
  public Action(int player, String playerName, ActionType type, int hintReceiver, boolean[] cards, Colour hint)
   ...
   * @throws IllegalActionException if the wrong ActionType is given
   */
  public Action(int player, String playerName, ActionType type, int hintReceiver, boolean[] cards, int hint)
      throws IllegalActionException {
    this(player, playerName, type);
    if (type != ActionType.HINT_VALUE)
      throw new IllegalActionException("Wrong parameters for action type");
    this.hintee = hintReceiver;
    this.cards = cards;
    this.value = hint;
  ...
  public int getCard() throws IllegalActionException {
    if (type != ActionType.PLAY && type != ActionType.DISCARD)
      throw new IllegalActionException("Card is not defined");
    return card;
  }
   ...
   * @throws IllegalActionException if the action type is not HINT_COLOUR or
   *                                HINT_VALUE
   **/
  public int getHintReceiver() throws IllegalActionException {
    if (type != ActionType.HINT_COLOUR && type != ActionType.HINT_VALUE)
      throw new IllegalActionException("Action is not a hint");
    return hintee;
  }

  /**
   * gets an array of booleans indicating the cards that are the subject of the
   * hint
   * 
   * @return an array of booleans, such that the ith value is true if and only if
   *         the ith card in the hintReceivers hand matches the hint (indexing
   *         from 0)
   * @throws IllegalActionException if the action type is not HINT_COLOUR or
   *                                HINT_VALUE
   **/
  ...
  public Colour getColour() throws IllegalActionException {
    if (type != ActionType.HINT_COLOUR)
      throw new IllegalActionException("Action is not a colour hint");
    return colour;
  }
  ...
  public int getValue() throws IllegalActionException {
    if (type != ActionType.HINT_VALUE)
      throw new IllegalActionException("Action is not a value hint");
    return value;
  }
  ...
  public String toString() {
    ...
    case HINT_COLOUR:
      ret = "Player " + playerName + "(" + player + ") gives the hint: \"Player " + hintee + ", cards at position"
          + (cards.length > 1 ? "s" : "");
