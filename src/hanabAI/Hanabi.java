...
public class Hanabi {
  ...
  public Hanabi(Agent[] agents) throws IllegalArgumentException {
    ...
    for (int i = 0; i < s.length; i++)
      s[i] = agents[i].toString();
    state = new State(s, deck);
  }

  /**
   * Plays the game. The agents will execute their strategies until the game is
   * complete and a number is returned.
   * 
  ...
  public static String critique(int score) {
    if (score == 0)
      return "Tragic: The pyrotechnicians are obliterated by their own incompetence.\n";
    if (score < 6)
      return "Horrible: boos from the crowd.\n";
    if (score < 11)
      return "Poor: a smattering of applause.\n";
    if (score < 16)
      return "Honourable: but no one will remember it.\n";
    if (score < 21)
      return "Excellent: the crowd is delighted.\n";
    if (score < 25)
      return "Extraordinary: no one will forget it.\n";
    else
      return "Legendary: adults and children alike are speechless, with starts in their eyes.\n";
  }
  ...
  public static void main(String[] args) {
    Agent[] agents = { new agents.BradicalAgent(), new agents.BradicalAgent(), new agents.BradicalAgent() };
